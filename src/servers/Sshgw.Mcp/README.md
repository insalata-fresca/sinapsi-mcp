...
| `SSHGW_AUTHZ_NATS_SEED_PATH` + `SSHGW_AUTHZ_NATS_NKEY` | no | — | Scoped publish-only NATS identity for **Q2 decision emission** (authorization plane, home-server `docs/61`). When BOTH are set, every `execute-command` authorization decision is published (fire-and-forget, best-effort) as a CloudEvent on `homelab.security.authz.q2.<verdict>.cse` so the control plane (Sinapsi Sentinel) shows the command-safety layer live. **Unset ⇒ no emission** (the MCP runs unchanged). The identity must be authorised to publish ONLY on `homelab.security.authz.>` — never a broad admin nkey. Uses the shared `NATS_*` connection env for URL/TLS. |
| `SSHGW_AUTHZ_EVENT_SOURCE` | no | `sshgw-mcp://authz/` | CloudEvents `source` URI stamped on emitted Q2 decisions. |
