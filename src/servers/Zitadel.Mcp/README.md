...
typed `HttpClient` (bearer held server-side, hard per-request timeout), validates every tool
parameter before any call, scrubs upstream errors of credentials, and exposes a 15-tool
...
It authenticates to ZITADEL in one of two env-selected modes, chosen fail-closed:

- **Service-account-key mode (preferred).** When `ZITADEL_SA_KEY_FILE` is set, the host loads a
  ZITADEL service-account JSON key and **self-mints + auto-refreshes a short-lived JWT bearer**
  via the RFC 7523 `jwt-bearer` grant — no long-lived credential is held. This is the stronger,
  canonical model: the same binary needs no static token.
- **PAT mode.** When only `ZITADEL_TOKEN` is set, the host attaches that static service-account /
  PAT bearer to every call (unchanged legacy behaviour).

If neither `ZITADEL_SA_KEY_FILE` nor `ZITADEL_TOKEN` is set the server **fails to start**, naming
both paths.
...
|------|------|----------------|
| Config | `ZitadelConfig.cs` | Bind + validate env into an immutable record; select the auth mode fail-closed (SA-key when `ZITADEL_SA_KEY_FILE` set, else PAT when `ZITADEL_TOKEN` set, else throw); fail-closed on a missing base-URL and on a non-numeric / out-of-range port or HTTP timeout. |
| Auth (SA-key) | `Auth/ServiceAccount.cs`, `Auth/JwtBearerTokenProvider.cs`, `Auth/SaKeyAuthHandler.cs` | Load the SA JSON key, self-sign an RS256 JWT assertion, exchange it at `/oauth/v2/token` for a short-lived access token (cached until `exp − 60 s`), and attach it — plus the `Host` + `X-Forwarded-Proto: https` headers ZITADEL needs over a LAN-bypass origin — via a `DelegatingHandler` on the `ZitadelClient` pipeline. Only wired in SA-key mode. |
...
The server holds no credential in source — every auth input comes from the environment at deploy.
Exactly one of `ZITADEL_SA_KEY_FILE` (preferred) or `ZITADEL_TOKEN` selects the auth mode; if
neither is set the server fails to start.

| Env var | Required | Default | Purpose |
|---------|:--------:|---------|---------|
| `ZITADEL_BASE_URL` | yes¹ | — | Instance / API root, e.g. `https://auth.example.com`. The `/management/v1/` API paths (and, in SA-key mode, `/oauth/v2/token`) are appended. |
| `ZITADEL_API_URL` | yes¹ | — | Alias for `ZITADEL_BASE_URL` — the API root the SA-key deploy provides (may be a LAN-bypass origin, e.g. `http://10.x.y.z:80`). Either var satisfies the requirement; the server **fails to start** if both are unset. |
| `ZITADEL_SA_KEY_FILE` | mode² | — | Path to a ZITADEL service-account JSON key (`{type, keyId, key, userId}`). **Its presence selects SA-key mode**: the host self-mints a short-lived JWT bearer and holds no long-lived credential. |
| `ZITADEL_ISSUER` | no | `ZITADEL_BASE_URL` | SA-key mode: the public issuer used as the `aud` of the minted JWT assertion + the value ZITADEL validates the token against. Defaults to the API root. |
| `ZITADEL_HOST_HEADER` | no | API-root host | SA-key mode: the `Host` header sent on API + mint calls (with `X-Forwarded-Proto: https`) so ZITADEL sees the public host even when the API root is a LAN-bypass IP. |
| `ZITADEL_TOKEN` | mode² | — | A static service-account / PAT bearer token, held server-side. **Selects PAT mode** when no SA key file is set. Inject at deploy; **never bake it in**. |
...
¹ `ZITADEL_BASE_URL` **or** `ZITADEL_API_URL` — at least one is required (they are aliases for the
API root). ² `ZITADEL_SA_KEY_FILE` **or** `ZITADEL_TOKEN` — exactly one selects the auth mode
(SA-key takes precedence); if neither is set the server fails to start.

## Run

SA-key mode (preferred — the host self-mints a short-lived JWT bearer, no long-lived credential):
...
PAT mode (static service-account bearer):
...
- **Fail-closed config + auth.** A base URL (`ZITADEL_BASE_URL` / `ZITADEL_API_URL`) is required,
  and an auth mode must be fully configured — the server throws on startup if neither
  `ZITADEL_SA_KEY_FILE` nor `ZITADEL_TOKEN` is set, rather than running with no auth. A
...
- **Preferred: no long-lived credential (SA-key mode).** With `ZITADEL_SA_KEY_FILE` set, the host
  holds only the service-account JSON key on disk and self-mints a **short-lived** JWT bearer
  (cached until `exp − 60 s`, auto-refreshed) via the RFC 7523 `jwt-bearer` grant. The SA private
  key is imported into a transient RSA to sign the assertion and never appears in a log line, a
  tool response, or an exception message; the minted access token is likewise never surfaced.
- **No secrets in source.** In PAT mode the static token lives in `ZITADEL_TOKEN`, injected at
  deploy, held server-side on the typed `HttpClient` and never read into a tool response.
...
- **Auth-mode selection + JWT mint** (`ZitadelAuthModeTests`): SA-key mode is selected when
  `ZITADEL_SA_KEY_FILE` is set (and wins even if a static token is also present), PAT mode when only
  `ZITADEL_TOKEN` is set, and **neither → fail-closed** naming both paths; the `ZITADEL_API_URL`
  alias + explicit issuer / host-header are bound; and the mint path is exercised end-to-end with a
  **real RSA key + a scripted `/oauth/v2/token` endpoint** — asserting the `jwt-bearer` grant, a
  three-part signed assertion, the `Host` + `X-Forwarded-Proto: https` headers, token caching across
  calls, and a non-2xx exchange surfaced as a `ZitadelApiException` (the SA private key / minted
  token never leaked).
