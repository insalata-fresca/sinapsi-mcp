...
/// discover it) without a hand-rolled direct API call. Path-segment params
/// (owner/repo) and the topic (which reaches a URL segment on add/remove) are
/// validated at the top of each tool via <see cref="SinapsiForgeValidation"/>;
/// upstream errors are scrubbed by <see cref="ForgeToolGuard"/>.
/// </summary>
[McpServerToolType]
public sealed class TopicsTools
{
    ...
    public static Task<object> ListRepoTopics(IForgeClient forge, string owner, string repo,
        CancellationToken ct = default)
        => ForgeToolGuard.RunAsync(
            () => SinapsiForgeValidation.ValidateOwnerRepo(owner, repo),
            async () => new { owner, repo, topics = await forge.ListRepoTopicsAsync(owner, repo, ct) });
    ...
    public static Task<object> AddRepoTopic(IForgeClient forge, string owner, string repo,
        [Description("The topic to add, e.g. \"knowledge\".")] string topic,
        CancellationToken ct = default)
        => ForgeToolGuard.RunAsync(
            () => SinapsiForgeValidation.ValidateOwnerRepo(owner, repo) ?? SinapsiForgeValidation.ValidateSegment(topic, "topic"),
            async () => new { owner, repo, topics = await forge.AddRepoTopicAsync(owner, repo, topic, ct) });
    ...
    public static Task<object> RemoveRepoTopic(IForgeClient forge, string owner, string repo,
        [Description("The topic to remove.")] string topic,
        CancellationToken ct = default)
        => ForgeToolGuard.RunAsync(
            () => SinapsiForgeValidation.ValidateOwnerRepo(owner, repo) ?? SinapsiForgeValidation.ValidateSegment(topic, "topic"),
            async () => new { owner, repo, topics = await forge.RemoveRepoTopicAsync(owner, repo, topic, ct) });
    ...
    public static Task<object> SetRepoTopics(IForgeClient forge, string owner, string repo,
        [Description("The complete desired set of topics; replaces whatever is currently set.")] IReadOnlyList<string> topics,
        CancellationToken ct = default)
        => ForgeToolGuard.RunAsync(
            () => SinapsiForgeValidation.ValidateOwnerRepo(owner, repo),
            async () => new { owner, repo, topics = await forge.SetRepoTopicsAsync(owner, repo, topics, ct) });
}
